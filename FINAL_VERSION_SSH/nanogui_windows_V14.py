...
INTRO_TEXTS = {
    ...
    "I2C": ("I2C  -  VL53L0X Distance Sensor",
            "Sensor VIN to pin 1 (3.3V), GND to pin 6, SCL to pin 5, SDA "
            "to pin 3. Hold a target 5 to 50 cm above the sensor - the "
            "test takes three distance readings and averages them."),
...
class NanoGUI:
    ...
    def _build_i2s_in_frame(self):
        ...
        self._back_button(self.i2sInFrame, self.i2s_in_back)
    ...
    def _build_i2c_frame(self):
        ...
        self.i2c_running_label = tk.Label(self.i2cFrame, text="",
                                          font=F_BODYB, bg=PANEL_BG, fg=INK,
                                          justify=tk.LEFT, wraplength=800)
        self.i2c_running_label.pack(anchor="w", padx=22)
        wrap = tk.Frame(self.i2cFrame, bg=PANEL_BG)
        wrap.pack(fill=tk.BOTH, expand=True, padx=22, pady=10)
        bar = tk.Scrollbar(wrap)
        bar.pack(side=tk.RIGHT, fill=tk.Y)
        self.i2c_output = tk.Text(wrap, font=F_MONO, height=12,
                                  bg="#fbfcfd", fg=INK, relief=tk.FLAT,
                                  highlightbackground=BORDER,
                                  highlightthickness=1, wrap=tk.WORD,
                                  yscrollcommand=bar.set)
        self.i2c_output.pack(fill=tk.BOTH, expand=True)
        bar.config(command=self.i2c_output.yview)
        self.i2c_output.config(state=tk.DISABLED)
        tk.Checkbutton(self.i2cFrame,
                       text="Yes - the distance readings looked correct",
                       variable=self.icvar, font=F_BODY, bg=PANEL_BG,
                       fg=INK, activebackground=PANEL_BG,
                       selectcolor=PANEL_BG).pack(anchor="w", padx=20)
        self._back_button(self.i2cFrame, self.i2c_back)
    ...
    def i2c_test(self):
        ...
        self.icvar.set(0)
        self.i2c_auto = None
        self.i2c_output.config(state=tk.NORMAL)
        self.i2c_output.delete("1.0", tk.END)
        self.i2c_output.config(state=tk.DISABLED)
        ...
        self._set_test_buttons(tk.DISABLED)
        self.add_log("I2C distance sensor test starting...")
        threading.Thread(target=self._i2c_worker, daemon=True).start()
    ...
    def _i2c_worker(self):
        ...
        try:
            ...
            for line in self._read_lines(channel, lambda: True):
                collected.append(line)
                self.root.after(0, self._i2c_line, line)
            try:
                channel.close()
            except Exception:
                pass
            ...
            collected.append(f"SSH ERROR: {e}")
        self.root.after(0, self._i2c_done, "\n".join(collected))

    def _i2c_line(self, line):
        self.i2c_output.config(state=tk.NORMAL)
        self.i2c_output.insert(tk.END, line + "\n")
        self.i2c_output.see(tk.END)
        self.i2c_output.config(state=tk.DISABLED)
        if line.startswith("Reading"):
            self.i2c_running_label.config(
                text="Taking distance readings...", fg=STATE_RUN)

    def _i2c_done(self, output):
        auto = any(l.strip() == I2C_PASS_LINE for l in output.splitlines())
        self.i2c_auto = auto
        if auto:
            self.i2c_running_label.config(
                text="Readings complete. Confirm the distances looked "
                     "correct, then check the box.", fg=STATE_PASS)
        else:
            self.i2c_running_label.config(
                text="Script reported a problem - see the output above.",
                fg=STATE_FAIL)
            self._log_failure_detail(output)
        self.add_log(f"I2C script result: {'PASS' if auto else 'FAIL'}",
                     "pass" if auto else "fail")
        self._set_test_buttons(tk.NORMAL)
    ...
    def i2c_back(self):
        ...
        if self.i2c_auto is None:
            self.add_log("I2C test: INCOMPLETE (left before finishing).")
            self._set_pill("I2C", "idle")
            self._set_button_state("I2C", "ready")
        elif self.i2c_auto and human_ok:
            self.add_log("I2C test: PASS (script + readings confirmed).",
                         "pass")
            self.log_local_result("I2C", True, "script pass + human confirmed")
            ...
            self.log_local_result("I2C", False, "script pass, not confirmed")
